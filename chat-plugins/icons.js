...
exports.commands = {
	...
	icon: function (target, room, user) {
		...
		if (target[1] === 'delete') {
			...
			if (Users(target[0]) && Users(target[0]).connected) Users(target[0]).popup("|modal||html|<b><font color='" + Wisp.hashColor(user.name) + "'>" + user.name + "</font></b> removed your icon.");
			return;
		}
		...
		if (Users(target[0]) && Users(target[0]).connected) Users(target[0]).popup("|modal||html|<b><font color='" + Wisp.hashColor(user.name) + "'>" + user.name + "</font></b> has set your icon as:-<img src='" + target[1] + "' width='32' height='32'");
