...
var commands = exports.commands = {
	...
	dexsearch: function (target, room, user, connection, cmd, message) {
		...
		if (showAll && searches.length === 0 && megaSearch === null && tppSearch === null) return this.sendReplyBox("No search parameters other than 'all' were found. Try '/help dexsearch' for more information on this command.");

		var dex = {};
		var tpp = require('../data/tpp').BattleTPP;
		for (var pokemon in Tools.data.Pokedex) {
			var template = Tools.getTemplate(pokemon);
			var megaSearchResult = (megaSearch === null || (megaSearch === true && template.isMega) || (megaSearch === false && !template.isMega));
			var tppSearchResult = (tppSearch === null || (tppSearch === true && tpp[template.id]) || (tppSearch === false && !tpp[template.id]));
			if (template.tier !== 'Unreleased' && template.tier !== 'Illegal' && (template.tier !== 'CAP' || (searches['tier'] && searches['tier']['cap'])) && megaSearchResult && tppSearchResult) {
