exports.BattleScripts = {
	...
	randomDoublesTeam: function(side) {
		...
		for (var i in this.data.FormatsData) {
			//if (this.data.FormatsData[i].viableDoublesMoves || this.data.FormatsData[i].viableMoves) {
			if (this.data.FormatsData[i].viableDoublesMoves) {
				keys.push(i);
			}
		}
		keys = keys.randomize();
		...
		for (var i=0; i<keys.length && pokemonLeft < 6; i++) {
			...
			// Not available on BW
			if (template.species === 'Pichu-Spiky-eared') continue;
			var set = this.randomDoublesSet(template, i, teamStats);
	...
	randomDoublesSet: function(template, i, teamStats) {
		if (i === undefined) i = 1;
		
		// Getting the template for the Pokemon
		template = this.getTemplate(template);
		if (!template.exists) template = this.getTemplate('unown');
		
		// We check for viable doubles moves. If there aren't, check viable singles moves, then check for learnset.
		var viableMoves = template.viableDoublesMoves || template.viableMoves;
		var moveKeys = Object.keys(viableMoves || template.learnset).randomize();
		var moves = [];
		var ability = '';
		var item = '';
		// The default EVs and IVs are the same as Random Battle
		...
		do {
			while (moves.length<4 && j<moveKeys.length) {
				...
				j++;
				switch (moveid) {
				// Can only have either wide guard or protect
				case 'wideguard':
					if (!hasMove['wideguard'] && !hasMove['protect']) {
						hasMove['wideguard'] = true;
					} else {
						continue;
					}
					break;
				case 'protect':
					if (!hasMove['wideguard'] && !hasMove['protect']) {
						hasMove['protect'] = true;
					} else {
						continue;
					}
					break;
				} // End of switch for moves
				moves.push(moveid);
			}
			...
			// Checks for attacking type and possible ability
			for (var k=0; k<moves.length; k++) {
				var move = this.getMove(moves[k]);
				var moveid = move.id;
				hasMove[moveid] = true;
			...
			// We check which one is most likely
			if (counter['mixedsetup'] > counter['specialsetup'] && counter['mixedsetup'] > counter['physicalsetup']) {
				setupType = 'Mixed';
			} else if (counter['specialsetup'] > counter['physicalsetup'] && counter['specialsetup'] > counter['mixedsetup']) {
				setupType = 'Special';
			} else if (counter['physicalsetup'] > counter['specialsetup'] && counter['physicalsetup'] > counter['mixedsetup']) {
				setupType = 'Physical';
			} else {
				// Mixed by default
				setupType = 'Mixed';
			...
			for (var k=0; k<moves.length; k++) {
				...
				case 'uturn': case 'voltswitch': case 'relicsong':
					...
					break;
				// Bit redundant to have both
				case 'lavaplume': // In doubles we choose lava plume over the rest
					if (hasMove['flamethrower'] || hasMove['overheat'] || hasMove['fireblast'] || hasMove['blueflare']) rejected = true;
					break;
				case 'overheat':
					if (hasMove['fireblast']) rejected = true;
					break;
				case 'icebeam':
					...
					break;
				case 'hydropump':
					if (hasMove['razorshell'] || hasMove['surf'] || hasMove['scald']) rejected = true;
					break;
				case 'aquatail': // Waterfall has flinch
				...
				case 'roar':
					...
					break;
				case 'recover':
					// Use Roost over Recover to confuse unaware opponents on typing
		...
		// Update team stats for moves
		if (hasMove['trickroom']) teamStats.trickroom = 1;
		if (hasMove['raindance']) teamStats.rain = 1;
		if (hasMove['sandstorm']) teamStats.sand = 1;
		if (hasMove['hail']) teamStats.hail = 1;
		if (hasMove['sunnyday']) teamStats.sun = 1;
		if (hasMove['gravity']) teamStats.gravity = 1;

		// Any moveset modification goes here. Abilities & items selection.
		{
			var abilities = [template.abilities['0']];
			if (template.abilities['1']) {
				abilities.push(template.abilities['1']);
			...
			if (abilities[1]) {
				...
				if (ability === 'Defiant' && !counter['Physical']) {
					rejectAbility = true;
				}
				if (ability === 'Moody' && template.id !== 'bidoof') {
					rejectAbility = true;
				...
				if ((abilities[0] === 'Intimidate' || abilities[1] === 'Intimidate' || abilities[2] === 'Intimidate') && !hasMove['machpunch']) {
					ability = 'Intimidate';
				}
			}
			
			// Update team stats based on ability
			if (ability === 'Drizzle') teamStats.rain = 1;
			if (ability === 'Sand Stream') teamStats.sand = 1;
			if (ability === 'Snow Warning') teamStats.hail = 1;
			if (ability === 'Drought') teamStats.sun = 1;
			
			if (hasMove['gyroball']) {
				ivs.spe = 0;
				evs.spe = 0;
			} else if (teamStats.trickroom) {
				ivs.spe = 0;
				evs.spe = 0;
			}
			
			// By default, it's Sitrus Berry
			item = 'Sitrus Berry';
			if (template.requiredItem) {
				item = template.requiredItem;
			// Important weather items
			} else if (teamStats.hail || teamStats.sandstorm) {
				item = 'Leftovers';
			// High-priority items
			} else if (ability === 'Imposter') {
				item = 'Choice Scarf';
			} else if (ability === 'Wonder Guard' || template.species === 'Zoroark') {
				item = 'Focus Sash';
			} else if (template.species === 'Unown') {
				item = 'Choice Specs';
			} else if ((template.species === 'Wynaut' || template.species === 'Wobbuffet') && hasMove['destinybond'] && Math.random()*2 > 1) {
				item = 'Custap Berry';
			} else if (template.species === 'Ludicolo') {
				item = 'Absorb Bulb';
			} else if (template.species === 'Cresselia' && !hasMove['Rest']) {
				item = 'Sitrus Berry';
			} else if (template.species === 'Chansey') {
				item = 'Eviolite';
			// Let's add powerful gem attacks
			} else if ((hasMove['closecombat'] || hasMove['superpower']) && template.types.indexOf('Fighting') >= 0) {
				item = 'Fighting Gem';
			} else if (hasMove['earthquake'] && template.types.indexOf('Ground') >= 0) {
				item = 'Ground Gem';
			} else if (teamStats.rain && (hasMove['hydropump'] || hasMove['surf'] || hasMove['waterfall'])) {
				var possibleItems = ['Water Gem', 'Wacan Berry'].randomize();
				if (hasMove['waterfall'] || hasMove['protect'] || hasMove['helpinghand']) {
					item = possibleItems[0];
				} else {
					possibleItems.push('Choice Specs', 'Choice Scarf');
					possibleItems.randomize();
					item = possibleItems[0];
				}
			} else if ((hasMove['eruption'] || hasMove['fireblast']) && template.types.indexOf('Fire') >= 0) {
				item = 'Fire Gem';
			} else if (hasMove['bulletpunch'] && template.types.indexOf('Steel') >= 0) {
				item = 'Steel Gem';
			} else if (hasMove['suckerpunch'] && template.types.indexOf('Dark') >= 0) {
				item = 'Dark Gem';
			...
			// Then, add berries
				...
				item = 'Chople Berry';
			} else if (this.getEffectiveness('Ground', template) >= 1) {
				item = 'Shuca Berry';
			} else if (this.getEffectiveness('Ice', template) >= 1) {
				item = 'Yache Berry';
			} else if (this.getEffectiveness('Grass', template) >= 2) {
				item = 'Rindo Berry';
			} else if (this.getEffectiveness('Fire', template) >= 1) {
				item = 'Occa Berry';
			// Other items now
			} else if (hasMove['trick'] && hasMove['gyroball'] && (ability === 'Levitate' || hasType['Flying'])) {
				item = 'Macho Brace';
				...
				item = 'Toxic Orb';
			} else if (hasMove['raindance'] && ability !== 'Drizzle') {
				item = 'Damp Rock';
			} else if (hasMove['sunnyday'] && ability !== 'Drought') {
				item = 'Heat Rock';
			} else if (hasMove['sandstorm'] && ability !== 'Sand Stream') {
				item = 'Smooth Rock';
			} else if (hasMove['hail'] && ability !== 'Snow Warning') {
				item = 'Icy Rock';
			} else if (ability === 'Magic Guard' && hasMove['psychoshift']) {
				item = 'Flame Orb';
			} else if (ability === 'Sheer Force' || ability === 'Magic Guard') {
				...
				item = 'Air Balloon';
			} else if (hasMove['waterspout']) {
				item = 'Choice Scarf';
			} else if (hasMove['substitute'] || ability === 'Moody') {
				item = 'Leftovers';
			} else if ((hasMove['flail'] || hasMove['reversal']) && !hasMove['endure'] && ability !== 'Sturdy') {
				item = 'Focus Sash';
			} else if (ability === 'Iron Barbs') {
		...
		// Scale the levels to balance fights
		var levelScale = {
			LC: 95,
			NFE: 90,
			'LC Uber': 86,
		...
		var customScale = {
			// Really bad Pokemon and jokemons
			Azurill: 99, Burmy: 99, Cascoon: 99, Caterpie: 100, Cleffa: 100, Combee: 99, Feebas: 99, Igglybuff: 100, Happiny: 100, Hoppip: 99,
			Kakuna: 99, Kricketot: 99, Ledyba: 99, Magikarp: 100, Metapod: 99, Pichu: 99, Ralts: 99, Sentret: 99, Shedinja: 99,
			Silcoon: 99, Slakoth: 99, Sunkern: 99, Tynamo: 99, Tyrogue: 99, Unown: 100, Weedle: 99, Wurmple: 99, Zigzagoon: 99,
			Clefairy: 95, Delibird: 95, "Farfetch'd": 95, Jigglypuff: 95, Kirlia: 95, Ledian: 95, Luvdisc: 95, Marill: 95, Skiploom: 95,
			Pachirisu: 90,
