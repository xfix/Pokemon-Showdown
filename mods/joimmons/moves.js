	...
	"blackice": {
		...
		effect: {
			...
			onSwitchIn: function (pokemon) {
				let typeMod = this.getEffectiveness('Ice', pokemon);
				let factor = 16;
	...
	gravity: {
		...
		effect: {
			...
			onModifyPokemon: function (pokemon) {
				...
				let disabledMoves = {bounce:1, fly:1, hijumpkick:1, jumpkick:1, magnetrise:1, skydrop:1, splash:1, telekinesis:1};
	...
	leechseed: {
		...
		effect: {
			...
			onResidual: function (pokemon) {
				let target = pokemon.side.foe.active[pokemon.volatiles['leechseed'].sourcePosition];
				if (!target || target.fainted || target.hp <= 0) {
					this.debug('Nothing to leech into');
					return;
				}
				let typeMod = this.getEffectiveness('Grass', pokemon);
				let factor = 8;
				if (typeMod === 1) factor = 4;
				if (typeMod >= 2) factor = 2;
				if (typeMod === -1) factor = 16;
				if (typeMod <= -2) factor = 32;
				let damage = this.damage(pokemon.maxhp / factor, pokemon, target);
				if (damage) {
	...
	rapidspin: {
		...
		self: {
			onHit: function (pokemon) {
				...
				let sideConditions = {spikes:1, toxicspikes:1, stealthrock:1, iciclerock:1};
	...
	spikes: {
		...
		effect: {
			...
			onSwitchIn: function (pokemon) {
				...
				let damageAmounts = [0, 2, 3, 4, 6, 8]; // 0, 1/12, 1/8, 1/6, 1/4, 1/3
