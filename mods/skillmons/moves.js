...
exports.BattleMovedex = {
	acupressure: {
		...
		onHit: function (target) {
			...
			let previousMax = 0;
			let found = 0;
			for (let i in target.baseStats) {
				if (target.baseStats[i] > previousMax && target.boosts[i] < 6) {
					toBoost = i;
					previousMax = target.baseStats[i];
					found++;
				}
			}
			if (found > 0) {
	...
	defog: {
		...
		onHit: function (target, source) {
			...
			let sideConditions = {reflect:1, lightscreen:1, safeguard:1, mist:1, spikes:1, toxicspikes:1, stealthrock:1, stickyweb:1};
	...
	gravity: {
		...
		effect: {
			...
			onModifyPokemon: function (pokemon) {
				...
				let disabledMoves = {bounce:1, fly:1, highjumpkick:1, jumpkick:1, magnetrise:1, skydrop:1, splash:1, telekinesis:1};
	...
	present: {
		...
		onBasePower: function (basePower, pokemon, target) {
			let percent = Math.ceil(target.hp * target.maxhp / 100);
			basePower = 60;
			if (percent > 75) {
				basePower = 120;
			} else if (percent > 50) {
